...
import { X, Lock, ShieldCheck, AlertCircle, LogIn, Sparkles } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if ((window as any).google) {
        ...
        const btnDiv = document.getElementById('googleSignInBtnModal');
        if (btnDiv) {
  ...
  const handleGoogleResponse = async (response: any) => {
    ...
    try {
      ...
      // Fallback local test user if backend offline
      const mockUser = {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-emerald-900/20 text-slate-100">
        ...
        <div className="mt-6 flex flex-col items-center gap-3">
          ...
          <div id="googleSignInBtnModal" className="w-full flex justify-center min-h-[44px]"></div>
